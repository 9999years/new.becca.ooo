import { $ } from "./fake"
import { listenChange, linkRange, linkEl } from "./fox_setup"

export var stepTimer = undefined;
export var secondsTimer = undefined;
export var seconds = 0;
export var time = {seconds: 0, minutes: 0, hours: 0};
export var numAnimals = 3;
export var steps = 0;
export var deaths = 0;
export var animalCount = 0;
export var worldHeight = 40;
export var worldWidth = 40;
export var world = [];
export var worldFacts = [];
export var animals = [];
export var data = [];
export var colors = [];
export var growAmount = 0.2; // 1 seems stable
export var outputWidth = 600;
export var outputHeight = 600;
export var runningMax = 100;
export var FPS = 60;
export var seed = 3;
export var worldCanvas = undefined
export var worldContext = undefined
export var statsCanvas = undefined
export var statsContext = undefined
export var graphCanvas = undefined
export var graphContext = undefined

const minMax = (a, b) => { return { min: a, max: b } }
...
function move() { //TODO for carnivores recursively look farther for prey (vision stat?)
	...
	var dy = 0;
	if (this.vore == "herb") {
		var highestValue = world[this.x][this.y];
		var lowestValue = world[this.x][this.y];
		var cumulative = 0;
		var square = 0; //store current square value for easy access
...
function output() {
	...
	for (var k = 0; k < data.length; k++) { //for each animal.type
		...
		var currentCountHeight = graphContext.canvas.height - (data[k][data[k].length - 1] - runningMin) / (runningMax - runningMin) * graphContext.canvas.height;
		...
		var speciesString = k == 0 ? "grass" : k + colors[k].vore.substr(0,1);
...
export function initialize() {
	...
	for (var i = 1; i <= numAnimals; i++) {
		const vore = random(0, 1) ? "carne" : "herb";
		colors[i] = {
			color: random(0, 360),
			vore: vore
		};
...
function initial_setup() {
    ...
    linkRange('grow', v => { growAmount = v }, false)
    linkRange('animalsNumber', v => { numAnimals = v })
    linkRange('worldSize', v => { worldWidth = worldHeight = v })

    //min ranges

    linkEl("hpMinNumberRange", value => { animalSettings.hp.min = value })
    linkEl("maxHPMinNumberRange", value => { animalSettings.maxHP.min = value })
    linkEl("lifespanMinNumberRange", value => { animalSettings.lifespan.min = value })
    linkEl("speedMinNumberRange", value => { animalSettings.speed.min = value })
    linkEl("reproductionRateMinNumberRange", value => { animalSettings.reproductionRate.min = value })
    linkEl("defenseMinNumberRange", value => { animalSettings.defense.min = value })
    linkEl("digestionRateMinNumberRange", value => { animalSettings.digestionRate.min = value })
    linkEl("camoflageMinNumberRange", value => { animalSettings.camoflage.min = value })
    linkEl("eatAmountMinNumberRange", value => { animalSettings.eatAmount.min = value })
    linkEl("attackMinNumberRange", value => { animalSettings.attack.min = value })

    //max ranges

    linkEl("hpMaxNumberRange", value => { animalSettings.hp.max = value })
    linkEl("maxHPMaxNumberRange", value => { animalSettings.maxHP.max = value })
    linkEl("lifespanMaxNumberRange", value => { animalSettings.lifespan.max = value })
    linkEl("speedMaxNumberRange", value => { animalSettings.speed.max = value })
    linkEl("reproductionRateMaxNumberRange", value => { animalSettings.reproductionRate.max = value })
    linkEl("defenseMaxNumberRange", value => { animalSettings.defense.max = value })
    linkEl("digestionRateMaxNumberRange", value => { animalSettings.digestionRate.max = value })
    linkEl("camoflageMaxNumberRange", value => { animalSettings.camoflage.max = value })
    linkEl("eatAmountMaxNumberRange", value => { animalSettings.eatAmount.max = value })
    linkEl("attackMaxNumberRange", value => { animalSettings.attack.max = value })

    //values

    //min values

    linkEl("hpMinNumber", value => { animalSettings.hp.min = value })
    linkEl("maxHPMinNumber", value => { animalSettings.maxHP.min = value })
    linkEl("lifespanMinNumber", value => { animalSettings.lifespan.min = value })
    linkEl("speedMinNumber", value => { animalSettings.speed.min = value })
    linkEl("reproductionRateMinNumber", value => { animalSettings.reproductionRate.min = value })
    linkEl("defenseMinNumber", value => { animalSettings.defense.min = value })
    linkEl("digestionRateMinNumber", value => { animalSettings.digestionRate.min = value })
    linkEl("camoflageMinNumber", value => { animalSettings.camoflage.min = value })
    linkEl("eatAmountMinNumber", value => { animalSettings.eatAmount.min = value })
    linkEl("attackMinNumber", value => { animalSettings.attack.min = value })
    //max values

    linkEl("hpMaxNumber", value => { animalSettings.hp.max = value })
    linkEl("maxHPMaxNumber", value => { animalSettings.maxHP.max = value })
    linkEl("lifespanMaxNumber", value => { animalSettings.lifespan.max = value })
    linkEl("speedMaxNumber", value => { animalSettings.speed.max = value })
    linkEl("reproductionRateMaxNumber", value => { animalSettings.reproductionRate.max = value })
    linkEl("defenseMaxNumber", value => { animalSettings.defense.max = value })
    linkEl("digestionRateMaxNumber", value => { animalSettings.digestionRate.max = value })
    linkEl("camoflageMaxNumber", value => { animalSettings.camoflage.max = value })
    linkEl("eatAmountMaxNumber", value => { animalSettings.eatAmount.max = value })
    linkEl("attackMaxNumber", value => { animalSettings.attack.max = value })
...
window.addEventListener('load', e => initial_setup())
