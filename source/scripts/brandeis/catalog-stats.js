...
function stacked_subject_courses_per_semester() {
    ...
    const subjects = [ 'HS', 'MUS', 'THA', 'ECON', 'MATH', 'BIOL']
    ...
    for(const [sem, courses] of COURSES_PER_SUBJECT) {
        const row = [sem]
        for(const subj of subjects) {
...
function subject_courses_per_semester(subj) {
    const axis = subj + ' courses'
    let [data, opts] = base_graph(axis, {title: axis + ' offered'})
    for(const row of COURSES_PER_SUBJECT) {
...
function subject_enrollments_per_semester(subj) {
    ...
    for(const row of ENROLLMENTS_PER_SUBJECT) {
        const sem = row[0]
...
function subject_class_size_per_semester(subj) {
    ...
    for(const row of AVG_STUDENTS_PER_SUBJECT) {
        const sem = row[0]
        data.addRow([sem, row[1][subj]])
    }
...
function draw() {
    ...
    const subj = $('subject_chooser').value
...
const AVG_STUDENTS_PER_SUBJECT = [
...
const SUBJECTS = ['AAAS', 'AAAS/ENG', 'AAAS/FA', 'AAAS/WGS', 'AAPI', 'AMST',
